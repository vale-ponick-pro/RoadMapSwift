...
// Задача: что будет результатом выполнения?

let items = [7, -3, 2, -8, 4, 9]
let result = items
    .sorted() // [-8, -3, 2, 4, 7, 9] ← сортировка по возрастанию
    .filter { $0 % 2 == 0 } // ← оставляем только чётные числа, порядок НЕ меняем [-8, 2, 4]
    .map { String($0) } // ← преобразуем числа в строки ["-8", "2", "4"]
    .reduce("") { $1 + $0 } // "42-8"

print(result) // 42-8

// Почему "42-8"?
// Шаги reduce с замыканием { текущий_элемент + аккумулятор }:
// 1) "" + "-8" = "-8" ❌ НЕТ!
// 1) "-8" + "" = "-8" ✔ ДА!
// 2) "2" + "-8" = "2-8"
// 3) "4" + "2-8" = "42-8"
...
// Проще запомнить:
// { $1 + $0 } = каждый следующий элемент ДОБАВЛЯЕТСЯ СЛЕВА
// ["-8", "2", "4"] → "4" + "2" + "-8" = "42-8"

let nums = [7, -3, 2, -8, 4, 9]
let res = nums
    .sorted() // по возрасающей [-8, -3, 2, 4, 7, 9]
    .filter { $0 % 2 == 0 } // массив четных чисел, порядок НЕ меняем [-8, 2, 4]
    .map { String($0) } // преобрадовать в массив строк ["-8", "2", "4"]
    .reduce("") { $0 + $1 } // объединяет все элементы коллекции в одно значение, начиная с начального (пустая строка "") и применяя замыкание к каждой паре. Замыкание имеет вид: { аккумулятор + текущий_элемент } ($0 + $1)

print(res) // -824 ??? "" + "-8" = "-8", затем "-8" + "2" = "-82", и наконец "-82" + "4" = "-824".

// Тот же массив после трёх операций: ["-8", "2", "4"]

// reduce("") { $0 + $1 } = просто склеиваем по порядку
// "" + "-8" = "-8"
// "-8" + "2" = "-82"
// "-82" + "4" = "-824"
...
/* Ключевое отличие:
 ...
 { $1 + $0 } = ОБРАТНЫЙ порядок (каждый новый элемент идёт СЛЕВА)

 { $0 + $1 } = ПРЯМОЙ порядок (простое сложение строк)
